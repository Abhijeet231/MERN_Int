...
const AgentDetails = () => {
  ...
  if (!agent) return <p className="text-center mt-10 text-gray-500">Loading agent...</p>;

  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-6">
      {/* 🧾 Agent Card */}
      <div className="bg-white shadow-md rounded-2xl p-8 max-w-md w-full border border-gray-100">
        <h2 className="text-2xl font-bold text-gray-800 mb-4 text-center">{agent.name}</h2>

        <div className="space-y-2 text-gray-700 text-sm">
          <p><span className="font-semibold">Email:</span> {agent.email}</p>
          <p><span className="font-semibold">Mobile:</span> {agent.mobile}</p>
          <p><span className="font-semibold">Total Tasks:</span> {agent.assignedTask?.length || 0}</p>
        </div>
...
<div className="mt-6">
  <h3 className="text-lg font-semibold text-gray-800 mb-3">Assigned Tasks</h3>
  ...
  {agent.assignedTask && agent.assignedTask.length > 0 ? (
    ...
    <p className="text-gray-500 text-sm italic">No assigned tasks yet.</p>
  )}
</div>
